...
function customJavaScript($) {
  // Remove the RPK's `nojs` class on <html> and replace it with
  // `hasjs`, which provides a hook in CSS for styling
  // JavaScript-dependent styles (see the `css/screen.css` file):
...
// This establishes the rpk object that sets the jQuery path and
// version, adds it to each page that calls this file
// (`js/site.js`), and sets up the page to load the custom
// JavaScript above:

rpk = (function() {
  // Set the URL/path and version of jQuery to use:
  var jQueryURL = "//ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js";
  // That will load the latest jQuery, 3.2.1, if not already
  // loaded.
  //  See https://developers.google.com/speed/libraries/#jquery
  // for a list of all versions of jQuery available from Google
  // Hosted Libraries.

  // Loading jQuery using the method below avoids the trouble of
  // writing a <script> tag for it on every HTML page, and makes
  // it easier in the future for you to replace jQuery with a
  // different library or write purely in native JavaScript, if
  // you choose.
  function loadjQuery() {
    var jQ;
    if (window.jQuery) {
